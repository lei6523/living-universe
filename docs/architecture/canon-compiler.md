...
v0.1 中 Canon Compiler 被描述为"核心工具方向"；v0.2 将其明确定位为 Canon Engine 的一个组成部分。Canon Engine 还包含实体解析、身份碰撞检测、并发主张检测、影响范围分析、认知状态分析等检查器。详见 [canon-engine.md](canon-engine.md)。
...
v0.2 补充：Canon AI 的定位是 Semantic Validator（语义验证器），不创造真相，只验证主张与已有世界现实的兼容性；硬规则由 Rule Engine（规则引擎）、Graph Query（图查询）、Database Constraint（数据库约束）处理。
